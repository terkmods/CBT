                ...
                var contentString = '<?= $data['0']->stadium_name ?>';
                    ...
                    function calcRoute() {
                        ...
                        var end = new google.maps.LatLng(<?= $data['0']->lat ?>, <?= $data['0']->long ?>);
                            ...
                            function handle NoGeolocation(errorFlag) {
                                if (errorFlag) {
...
<script>
        ...
        $.getJSON("http://cbt.backeyefinder.in.th/stadium/showcomment/" + st_id, function (data) {
            ...
            if (data.length != 0) { 
                ...
                $(data).each(function (k, v) {
                    ...
                    html = '                            <div class="bubble-list no nja">' +
    ...
    $(function () {
        ...
        $("#addcomment").click(function () {
            ...
            if (content.length > 0) {
                $.ajax({
                    type: "POST",                             url: "http://cbt.backeyefinder.in.th/stadium/addcomment/" + st_id,
                    data: {content: content}
                    }).done(function (msg) {                 //                    if($("#noann").length!=0){
                    //                        $("#noann").remove();
                    //                    }
                    //                    var html = '<tr class="even"><td><div class="media" id="newAnnounce"><a class="pull-left" href="#"><img class="img-circle" width="64" src="' + pic + '"></a><div c lass="media-body"><h4 class="media-heading"><small class="text-muted">' + fullname + '</small><small class="pull-right">' + dateSt + '</small></h4><p>' + $("#content").val() + '</p></div></div></td></tr>';
        ...
        var d = new Date();                     var dateSt = d.getFullYear() + "-" + ('0' + (d.getMonth() + 1)).slice(-2) + "-" + ('0' + d.getDate()).slice(-2) + " " + d.getHours()  + ":" + d.getMinutes() + ":" + d.getMilliseconds();
